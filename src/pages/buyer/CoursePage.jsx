...
const CoursePage = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      ...
      try {
        ...
        if (data) {
          setCourse({
            ...
            price: Number(data.price || data.monthly_price || 0),
      ...
      const found = mockCourses.find(c => c.id === Number(id)) || mockCourses[0];
      setCourse({
        ...found,
        description: "Master traditional heritage techniques passed down through generations of master artisans.",
        duration: "4.5 Hours • On-demand Video",
        level: "Beginner to Advanced",
        enrolledCount: 340
      });
        ...
        <div className="course-video-card">
          ...
          <div className="pricing-card-body">
            <div className="course-price-row">
              {isPremium ? (
                <span className="price-tag" style={{ color: '#22c55e' }}>FREE with VIP Pass</span>
              ) : (
                <>
                  <span className="price-tag">₹{course.price.toLocaleString('en-IN')}</span>
                  <span className="strike-price">₹{(course.price * 2.5).toLocaleString('en-IN')}</span>
                  <span className="discount-badge">Free with VIP</span>
                </>
              )}
            </div>
            ...
            <button 
              ...
              style={{ background: isPremium ? 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)' : 'linear-gradient(135deg, #FFD700 0%, #E0A80C 100%)', color: isPremium ? '#fff' : '#1e1104' }}
              onClick={handleEnroll}
            >
              {isPremium ? '▶ Watch Masterclass Now' : '👑 Unlock Masterclass with Premium VIP'}
            </button>

            <ul className="perks-list">
              <li>✓ Included with CraftConnect VIP Membership</li>
